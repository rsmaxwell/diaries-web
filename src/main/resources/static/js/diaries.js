...
(() => {
  ...
  const initialiseMonthReader = () => {
    ...
    const updateViewBox = () => {
      ...
      view.x = clamp(view.x, 0, Math.max(0, page.width - view.width));
      view.y = clamp(view.y, 0, Math.max(0, page.height - view.height));
    ...
    viewer.querySelectorAll('[data-viewer-action]').forEach(button => {
      button.addEventListener('click', () => {
        ...
        if (action === 'fit-page' || action === 'reset') fitPage();
    ...
    surface.addEventListener('pointermove', event => {
      ...
      if (values.length === 1 && previousPointer) {
        const bounds = surface.getBoundingClientRect();
        view.x -= (event.clientX - previousPointer.x) * view.width / Math.max(1, bounds.width);
        view.y -= (event.clientY - previousPointer.y) * view.height / Math.max(1, bounds.height);
        previousPointer = { x: event.clientX, y: event.clientY };
        updateViewBox();
